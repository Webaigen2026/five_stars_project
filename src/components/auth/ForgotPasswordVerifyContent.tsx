...
import { maskEmailForDisplay } from "../../lib/password-reset";
...
export default function ForgotPasswordVerifyContent() {
  ...
  function handleChange(index: number, value: string) {
    ...
    const advanceTo = Math.min(index + chars.length, CODE_LENGTH - 1);
    focusIndex(advanceTo);
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!email) {
      setError("Missing email. Please start again from Forgot password.");
      return;
    }
  ...
  async function handleResend() {
    if (isResending || cooldown > 0 || !email) {
      return;
    }
    ...
    try {
      ...
      setInfo(
        "If an account exists for that email, we've sent a password reset code. This code expires in 10 minutes."
      );
