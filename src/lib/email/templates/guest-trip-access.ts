/**
 * Guest Find My Trip access code email (D15.2).
 */

import { CUSTOMER_BRAND, CUSTOMER_BRAND_MARK } from "../../brand";

export const GUEST_TRIP_ACCESS_EMAIL_SUBJECT = `Your ${CUSTOMER_BRAND} trip access code`;

export function buildGuestTripAccessCodeEmail(input: {
  code: string;
  expiresInLabel?: string;
}) {
  const expiresInLabel = input.expiresInLabel ?? "10 minutes";
  const subject = GUEST_TRIP_ACCESS_EMAIL_SUBJECT;
  ...
  const text = [
    ...
    "Access your trip",
    "",
    `We received a request to access your ${CUSTOMER_BRAND} booking.`,
    "",
    `Your verification code is: ${input.code}`,
    "",
    `This code expires in ${expiresInLabel}.`,
    "",
    "If you didn't request access to this booking, you can ignore this email.",
    "",
    CUSTOMER_BRAND,
    "fivestarsfly.com",
  ].join("\n");
          ...
          <tr>
            <td style="padding:28px 28px 8px 28px;">
              ...
              <p style="margin:16px 0 0 0;font-size:16px;line-height:1.6;color:#334155;">
                We received a request to access your ${escapeHtml(CUSTOMER_BRAND)} booking.
              </p>
            </td>
          </tr>
          <tr>
